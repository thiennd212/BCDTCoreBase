...
[ApiController]
[Route("api/v1/forms/{formId:int}/sheets/{sheetId:int}/dynamic-regions")]
[Authorize]
[Produces("application/json")]
public class FormDynamicRegionsController : ControllerBase
{
    private readonly IFormDynamicRegionService _service;

    public FormDynamicRegionsController(IFormDynamicRegionService service) => _service = service;

    [HttpGet]
    [ProducesResponseType(typeof(ApiSuccessResponse<List<FormDynamicRegionDto>>), StatusCodes.Status200OK)]
    public async Task<IActionResult> GetList(int formId, int sheetId, CancellationToken cancellationToken)
    {
        ...
        return Ok(new ApiSuccessResponse<List<FormDynamicRegionDto>>(result.Data!));
    }

    [HttpGet("{regionId:int}")]
    [ProducesResponseType(typeof(ApiSuccessResponse<FormDynamicRegionDto>), StatusCodes.Status200OK)]
    ...
    public async Task<IActionResult> Get(int formId, int sheetId, int regionId, CancellationToken cancellationToken)
    {
        ...
        if (result.Data == null)
            return NotFound(new ApiErrorResponse("NOT_FOUND", "Vùng chỉ tiêu động không tồn tại."));
        return Ok(new ApiSuccessResponse<FormDynamicRegionDto>(result.Data));
    }

    [Authorize(Policy = "FormStructureAdmin")]
    [HttpPost]
    [ProducesResponseType(typeof(ApiSuccessResponse<FormDynamicRegionDto>), StatusCodes.Status201Created)]
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    public async Task<IActionResult> Create(int formId, int sheetId, [FromBody] CreateFormDynamicRegionRequest request, CancellationToken cancellationToken)
    {
        ...
        return CreatedAtAction(nameof(Get), new { formId, sheetId, regionId = result.Data!.Id }, new ApiSuccessResponse<FormDynamicRegionDto>(result.Data!));
    }
    ...
    [ProducesResponseType(typeof(ApiSuccessResponse<FormDynamicRegionDto>), StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    public async Task<IActionResult> Update(int formId, int sheetId, int regionId, [FromBody] UpdateFormDynamicRegionRequest request, CancellationToken cancellationToken)
    {
