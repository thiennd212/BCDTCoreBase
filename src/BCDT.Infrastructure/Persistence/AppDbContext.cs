...
public class AppDbContext : DbContext
{
    ...
    public DbSet<FormRowFormulaScope> FormRowFormulaScopes => Set<FormRowFormulaScope>();
    public DbSet<FormCellFormula> FormCellFormulas => Set<FormCellFormula>();
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        ...
        modelBuilder.Entity<FormColumn>(e =>
        {
            ...
            e.Property(x => x.ExcelColumn).HasMaxLength(10).IsRequired(false);
            e.Property(x => x.LayoutOrder).HasDefaultValue(0);
        ...
        modelBuilder.Entity<FormRow>(e =>
        {
            ...
            e.Property(x => x.IsEditable).HasDefaultValue(true);
            e.Property(x => x.IsRequired).HasDefaultValue(false);
            e.Property(x => x.Formula).HasMaxLength(1000);
            e.HasOne<FormSheet>().WithMany().HasForeignKey(x => x.FormSheetId);
        ...
        modelBuilder.Entity<FormRowFormulaScope>(e =>
        {
            e.ToTable("BCDT_FormRowFormulaScope");
            e.HasKey(x => x.Id);
            e.HasOne<FormRow>().WithMany().HasForeignKey(x => x.FormRowId).OnDelete(DeleteBehavior.Cascade);
            e.HasOne<FormColumn>().WithMany().HasForeignKey(x => x.FormColumnId).OnDelete(DeleteBehavior.Cascade);
            e.HasIndex(x => new { x.FormRowId, x.FormColumnId }).IsUnique();
        });

        modelBuilder.Entity<FormCellFormula>(e =>
        {
            e.ToTable("BCDT_FormCellFormula");
            e.HasKey(x => x.Id);
            e.Property(x => x.Formula).HasMaxLength(1000);
            e.HasOne<FormSheet>().WithMany().HasForeignKey(x => x.FormSheetId).OnDelete(DeleteBehavior.Cascade);
            e.HasOne<FormColumn>().WithMany().HasForeignKey(x => x.FormColumnId).OnDelete(DeleteBehavior.NoAction);
            e.HasOne<FormRow>().WithMany().HasForeignKey(x => x.FormRowId).OnDelete(DeleteBehavior.NoAction);
            e.HasIndex(x => new { x.FormColumnId, x.FormRowId }).IsUnique();
        });
    }
}
