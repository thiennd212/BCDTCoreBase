...
**Ngày cập nhật:** 2026-03-03 · **Version 2.85** · **Rà soát lần 130** (Sprint 8: S8.1 ✅, S8.2 ❌ dev limit, S8.3 ✅ PRs merged, S8.4 ⚠️ Phương án C done – breaking point ~250 VU dev machine; Phương án B staging còn chờ)
...
| **Sprint 8 – S8.4** ⚠️ | **P5 Stress – Phương án C (done) + Phương án B (chờ staging)** | **Phương án C xong (2026-03-03):** Ramp 200→300→500 VU trên dev machine (i5-10210U, 32GB RAM). **Breaking point ~250 VU**: p95 nhảy 2.01s→8.12s khi vượt 200→300 VU (4× degradation). 500 VU: p95=14.71s, error=0.54% (graceful, không crash). Kết quả tham khảo – giới hạn hardware laptop. **Phương án B** (staging 8 core) vẫn cần để có số liệu production-like. `docs/load-test/STAGING_SETUP.md` sẵn sàng. | `docs/load-test/W17_LOAD_TEST_CCU.md` mục P5-Ramp |
