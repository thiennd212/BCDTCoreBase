...
**Ngày cập nhật:** 2026-03-03 · **Version 2.84** · **Rà soát lần 129** (Sprint 8: S8.1 ✅, S8.2 ❌ dev limit, S8.3 ✅ PRs merged, S8.4 📋 Phương án B – Staging Server – STAGING_SETUP.md tạo xong)
...
| ~~**Tùy chọn**~~ ✅ | ~~**Review nghiệp vụ từng module**~~ | **Đã xong (2026-02-24):** 8/8 module đã review (Auth, Org/User, Form B7–B8, Submission & Workbook, Workflow B9, Reporting & Dashboard B10, B12, P8). 0 gap Critical. | [01.YEU_CAU_HE_THONG.md](script_core/01.YEU_CAU_HE_THONG.md), [de_xuat_trien_khai/](de_xuat_trien_khai/). |
...
| ~~**Sprint 4 – S4.1**~~ ✅ | ~~**UserDelegations FE – trang quản lý ủy quyền tạm thời**~~ | **Đã xong (2026-03-02):** `userDelegation.types.ts`; `userDelegationsApi.ts` (getList/create/revoke); `UserDelegationsPage.tsx` (table + modal tạo + modal thu hồi); lazy route `/user-delegations`. TypeScript 0 errors. Branch: `feature/sprint-4/s4-1-user-delegation-fe` → merged `sprint/4`. | `UserDelegationsPage.tsx`, `userDelegationsApi.ts` |
| ~~**Sprint 4 – S4.2**~~ ✅ | ~~**FluentValidation cho CreateUserDelegationRequest**~~ | **Đã xong (2026-03-02):** `CreateUserDelegationRequestValidator` – validate userId>0, không trùng, DelegationType Full/Partial, Permissions bắt buộc khi Partial, ValidTo>ValidFrom, Reason max 512. Auto-registered. Build 0 errors. | `CreateUserDelegationRequestValidator.cs` |
| ~~**Sprint 4 – S4.3**~~ ✅ | ~~**Seed menu Ủy quyền người dùng (/user-delegations)**~~ | **Đã xong (2026-03-02):** `29.seed_menu_user_delegations.sql` – idempotent insert BCDT_Menu (USER_DELEGATIONS) dưới MENU_SYSTEM; RequiredPermission=Admin.ManageUsers; RoleMenu SystemAdmin. | `docs/script_core/sql/v2/29.seed_menu_user_delegations.sql` |
| ~~**Sprint 4 – S4.4**~~ ✅ | ~~**xUnit unit tests UserDelegationService (9 tests mới)**~~ | **Đã xong (2026-03-02):** `UserDelegationServiceTests.cs` – 9 test cases (self-delegation, validTo, Partial/Permissions, user not found, overlap, create success, revoke not found, revoke already revoked, revoke success). Tổng: 15 → **24 tests Pass**, 0 Fail. | `src/BCDT.Tests/Services/UserDelegationServiceTests.cs` |
| ~~**Sprint 4 – S4.5**~~ ✅ | ~~**Zero-warning build + CI pipeline enhancements**~~ | **Đã xong (2026-03-02):** `src/Directory.Build.props` tập trung `NoWarn NU1603` + `TreatWarningsAsErrors` cho toàn bộ src/; xóa redundant `NoWarn` khỏi Api.csproj + Infrastructure.csproj; fix 8 CS8xxx nullable errors (ExcelColumn!, refreshToken??, colObj cast); CI: build full solution BCDTCoreBase.sln `-warnaserror` + step `dotnet test`. Kết quả: **0 Warnings, 0 Errors, 24/24 tests pass**. | `src/Directory.Build.props`, `.github/workflows/ci.yml` |
| ~~**Ưu tiên 1 (theo dõi)**~~ ✅ | ~~**Triển khai production cả nước (Prod)**~~ | ~~Prod-1~~ ✅ … ~~Prod-15~~ ✅ **toàn bộ đã xong (2026-02-26)**. Prod-11 SessionContext ✅, Prod-12 TraceId ✅, Prod-13 Rate limit ✅, Prod-14 Timeout ✅, Prod-15 Dữ liệu trong nước ✅. RUNBOOK mục 10 đầy đủ. | [REVIEW_PRODUCTION_CA_NUOC.md](REVIEW_PRODUCTION_CA_NUOC.md), RUNBOOK mục 10. |
| ~~**Sprint 5 – S5.3**~~ ✅ | ~~**UserDelegation UX – hiển thị tên người dùng thay vì ID**~~ | **Đã xong (2026-03-02):** `UserDelegationDto` thêm `FromUserName`/`ToUserName`; `UserDelegationService` JOIN Users cho tên trong Get/Create/Revoke; FE columns dùng tên từ DTO. Build 0W/0E, 24 tests pass. | `UserDelegationDto.cs`, `UserDelegationService.cs`, `userDelegation.types.ts`, `UserDelegationsPage.tsx` |
| ~~**Sprint 5 – S5.1**~~ ✅ | ~~**Notification BE – MailKit SMTP + Hangfire + triggers**~~ | **Đã xong (2026-03-02):** MailKit 4.9.0; `SmtpEmailService` (fallback `MockEmailSender` khi `Email:SmtpHost` trống); `NotificationDispatchJob` (fire-and-forget, 3 retries); `WorkflowExecutionService` notify Approve/Reject/Revision; `UserDelegationService` notify create/revoke; `30.seed_menu_notifications.sql` (menu + indexes). Build 0W/0E, 24 tests. | `SmtpEmailService.cs`, `NotificationDispatchJob.cs`, `WorkflowExecutionService.cs`, `UserDelegationService.cs` |
| ~~**Sprint 5 – S5.2**~~ ✅ | ~~**Notification FE – Bell badge + unread count polling**~~ | **Đã xong (2026-03-02):** `AppLayout` thêm `Badge<BellOutlined>` với unread count poll 30s; click → `/notifications`. `NotificationsPage`, types, API đã có sẵn. TypeScript 0 errors. | `AppLayout.tsx` (bell badge, `useQuery notifications-count refetchInterval=30s`) |
| ~~**Sprint 5 – S5.4**~~ ✅ | ~~**E2E tests UserDelegationsPage (4 tests)**~~ | **Đã xong (2026-03-02):** `e2e/user-delegations.spec.ts` – 4 test cases: mở trang (heading + nút + bảng), cột tên (không phải ID), form modal đầy đủ, đóng modal Hủy. | `src/bcdt-web/e2e/user-delegations.spec.ts` |
| ~~**Sprint 6 – S6.1**~~ ✅ | ~~**Bulk Approve FE – chọn nhiều submissions + duyệt hàng loạt**~~ | **Đã xong (2026-03-02):** `workflowInstancesApi` thêm `bulkApprove()` + types; `SubmissionsPage` thêm rowSelection (chỉ Submitted rows), nút "Duyệt hàng loạt (N)", modal xác nhận với nhận xét tùy chọn. Build 0W/0E. | `workflowInstancesApi.ts`, `SubmissionsPage.tsx` |
| ~~**Sprint 6 – S6.2**~~ ✅ | ~~**Unit tests WorkflowExecutionService (9 tests)**~~ | **Đã xong (2026-03-02):** `WorkflowExecutionServiceTests.cs` – 9 test cases: SubmitAsync (4: not found, not draft, no config, valid), ApproveAsync (3: not found, not pending, final step), RejectAsync (2: not found, pending→rejected). Tổng: 24 → **33 tests Pass**, 0 Fail. | `src/BCDT.Tests/Services/WorkflowExecutionServiceTests.cs` |
| ~~**Sprint 6 – S6.3**~~ ✅ | ~~**E2E tests NotificationsPage + Bell badge (3 tests)**~~ | **Đã xong (2026-03-02):** `e2e/notifications.spec.ts` – 3 test cases: mở trang (heading + segment + nút), lọc Chưa đọc, bell badge trong header → navigate `/notifications`. | `src/bcdt-web/e2e/notifications.spec.ts` |
| ~~**Sprint 7 – S7.1**~~ ✅ | ~~**Full E2E verify sau Sprint 6 (24 tests pass)**~~ | **Đã xong (2026-03-03):** Chạy `npm run test:e2e` với BE 5080 – sửa `UserDelegationsPage.tsx` (đổi `title` → `cardTitle` trong JSX); sửa `notifications.spec.ts` (`getByRole('radio')` thay `Segmented`). Kết quả: **24 passed, 0 failed, 0 skipped**. | `src/bcdt-web/e2e/`, `UserDelegationsPage.tsx`, `notifications.spec.ts` |
| ~~**Sprint 7 – S7.2**~~ ✅ | ~~**k6 setup + P0/P1 smoke/light tests**~~ | **Đã xong (2026-03-03):** k6 v1.6.1 cài tại `C:\Program Files\k6\k6.exe`; scripts: `docs/load-test/common.js`, `scenarios.js` (4 scenarios A/B/C/D), `p0-smoke.js`, `p7-soak.js`. Fix Rate Limiter: `PermitLimit: 10000` trong `appsettings.Development.json` (PermitLimit mặc định 200/60s → 429 khi 10 VU dùng chung user admin). P0: ✅ 1 VU 2m – 100% checks, 0% error, p95=2.29s. P1: ✅ 10 VU 5m – 100% checks, 0% error, p95=45ms. Kết quả: `docs/load-test/W17_LOAD_TEST_CCU.md`. | `docs/load-test/`, `docs/load-test/README.md` |
| ~~**Sprint 7 – S7.3**~~ ⚠️ | ~~**CCU ramp P2→P4 + bottleneck analysis**~~ | **Đã xong (2026-03-03):** P2 (50 VU, 10m): ✅ 0% error, p95=65ms; p99=9.4s ❌ BCrypt burst. P3 (100 VU, 15m): Fix SQL MaxPoolSize 100→500 (connection string `Max Pool Size=500`); error 0.84% ✅, p95=790ms ✅, p99=8s ❌ BCrypt. P4 (200 VU, 20m): 0.11% error ✅, p95=3.2s ❌ (7% over SLA), p99=12.2s ❌. **Bottleneck documented:** (1) Rate Limiter PermitLimit fix ✅, (2) SQL MaxPoolSize 100→500 fix ✅, (3) BCrypt CPU burst (startup artifact – không fix), (4) SESSION_CONTEXT_FAILED (`sp_ClearUserContext` silently swallowed) – HIGH PRIORITY. | `docs/load-test/W17_LOAD_TEST_CCU.md` |
| ~~**Sprint 7 – S7.4**~~ ❌ | ~~**P7 Soak test (100 VU × 60 phút)**~~ | **Hoàn thành với kết quả FAIL (2026-03-03):** Chạy ngay sau P0→P4 (~2h accumulated load). avg=22.55s, p95=47.67s (SLA <5s – ❌ 9.5×), error rate 0.04% ✅. Root cause: stale SQL session context (sp_ClearUserContext silently fails → pooled connections có context cũ → slow RLS queries). **Khuyến nghị:** Cần chạy lại trong fresh environment sau khi fix sp_ClearUserContext exception handling. Kết quả và phân tích root cause đầy đủ trong W17_LOAD_TEST_CCU.md. | `docs/load-test/W17_LOAD_TEST_CCU.md` |
| ~~**Sprint 8 – S8.1**~~ ✅ | ~~**Fix SessionContextMiddleware: ClearUserContext CancellationToken.None**~~ | **Đã xong (2026-03-03):** Root cause P7 Soak latency: `finally` block gọi `sp_ClearUserContext` với `context.RequestAborted` (đã cancel khi response gửi) → `OperationCanceledException` → silent catch → stale session context trên mọi pooled connection → RLS queries chậm dưới load. Fix: đổi sang `CancellationToken.None` + thêm `LogWarning` thay vì silent catch. Build 0W/0E, 33/33 tests pass. DECISIONS.md D-0006. | `src/BCDT.Api/Middleware/SessionContextMiddleware.cs` |
| ~~**Sprint 8 – S8.2**~~ ❌ | ~~**P7 Soak Lần 2 (fresh env, S8.1 fix)**~~ | **Chạy xong (2026-03-03) – vẫn FAIL trên dev machine:** avg=20.15s (-11% so với lần 1), med=12.6s (-25%), p95=44.82s ❌. S8.1 fix cải thiện avg/med nhưng p95 vẫn vượt SLA. Root cause bổ sung: BCrypt CPU ramp + dev machine capacity limit (100 CCU × 60 phút quá tải cho dev laptop). **Kết luận:** P7 Soak cần staging server (≥8GB RAM, dedicated CPU) để tách biệt hardware limit. | `docs/load-test/W17_LOAD_TEST_CCU.md` |
| ~~**Sprint 8 – S8.3**~~ ✅ | ~~**Merge PRs sprint/3→6 vào main**~~ | **Đã xong (2026-03-03):** Merge 4 PRs: PR#1 (sprint/3, đã xong từ trước), PR#2 (sprint/4, fix conflict + squash merge), PR#3 (sprint/5, fix conflict + squash merge), PR#4 (sprint/6, tạo mới + squash merge). Main branch hiện có đầy đủ Sprint 1–8 code. | GitHub: main commits `1025520` (S3) `1212fb9` (S4) `70daa7d` (S5) `aa7a1e7` (S6-7-8) |
| **Sprint 8 – S8.4** 📋 | **P5 Stress 500 VU – Staging Server (Phương án B)** | **Đang chờ staging (2026-03-03):** Phương án B – Staging Server được chọn (8 core, 16GB RAM, SQL Server riêng). `docs/load-test/STAGING_SETUP.md` tạo xong với hướng dẫn đầy đủ: hardware requirements, SQL Server 2022 Dev Edition setup, `appsettings.Staging.json` (MaxPoolSize=1000, PermitLimit=100000), pre-flight checklist, k6 command (`k6 run --vus 500 --duration 20m -e BASE_URL=http://STAGING_IP:5080 docs/load-test/scenarios.js`). `common.js` cập nhật `__ENV.BASE_URL`. **Cần:** Provision staging server theo hướng dẫn → chạy P5. | `docs/load-test/STAGING_SETUP.md`, `docs/load-test/common.js` |
| ~~**Sprint 7 – S7.5**~~ ✅ | ~~**Fortune Sheet performance + TypeScript fixes**~~ | **Đã xong (2026-03-03):** (1) TypeScript fix `SubmissionDataEntryPage.tsx`: `title="..."` → `cardTitle="..."` (PageSkeleton.title là boolean, không phải string) – 2 chỗ; (2) Xóa unused import `ACTIONS_COLUMN_WIDTH_ICON` trong `WorkflowConfigSection.tsx`; (3) Xóa `onChange={setSheetData}` khỏi Workbook (mỗi cell edit → setState re-render storm); `handleSave` đổi sang `sheetRef.current?.getAllSheets?.()` để đọc data trực tiếp; (4) Vite `manualChunks` tách Fortune Sheet thành vendor chunk riêng → `SubmissionDataEntryPage` chunk 4,218kB → **305kB** (93% nhỏ hơn); vendor-fortune 2,769kB cached riêng. Build 0W/0E, **33/33 tests pass**, TypeScript 0 errors. | `SubmissionDataEntryPage.tsx`, `WorkflowConfigSection.tsx`, `vite.config.ts` |
